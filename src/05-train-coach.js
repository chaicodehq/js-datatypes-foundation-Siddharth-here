...
export function findPassenger(passengers, name) {
  ...
  return passengers.find((passengers) => {
    return passengers.name.toLowerCase() === name.toLowerCase();
  });
}
...
export function getPassengerIndex(passengers, name) {
  ...
  return passengers.findIndex((passengers) => {
    return passengers.name.toLowerCase() === name.toLowerCase();
  });
}
...
export function isAnyWaitlisted(passengers) {
  ...
  return passengers.some((passengers) => {
    return passengers.status === "waitlisted";
  });
}
...
return passengers.every((passengers) => {
    return passengers.status === "confirmed";
  });
...
return passengers.filter((passengers) => {
    return passengers.status === "waitlisted";
  });
}
