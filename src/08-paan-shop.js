...
export function updatePrices(menu, increase) {
  ...
  const updateEntries = Object.entries(menu).map(([item, price]) => {
...
export function mergeDailySpecials(regularMenu, specialsMenu) {
  ...
  const isSpecialsValid =
    typeof specialsMenu === "object" && specialsMenu !== null;

  if (!isRegularValid && !isSpecialsValid) return {};

  const rMenu = isRegularValid ? regularMenu : {};
  const sMenu = isSpecialsValid ? specialsMenu : {};
